...
phase1model <-function(seed = 1234, patlist = NULL, patID_act = NULL,
  ...
  #' @param patlist A list of the patient treatment records,
  ...
  #' @param cycle_act A vector records the current cycle of patID_act.
  #' @param dlt_act A vector indicates whether a dlt is observed in current cycle for current patients
  ...
  #' @param effcy.flag Whether efficacy data is considered in the model fitting or not
  ...
  if(is.null(patID_act)){
    ...
    if(DLT.drop.flag == T){
      act.index <- which((cycle_act + 1) <= 6 & dlt_act == 0)
    } else {
      act.index <- which((cycle_act + 1) <= 6)
    }
  }
  ...
  if(effcy.flag == F){
    ...
    if(ICD.flag == F){
      ...
      if(n.cohort == Max.cohort){
        if(length(act.index) != 0){
          cat("No more new cohort, recommend dose for current paitents for next cycle based on longitudinal toxicity\n\n\n")
        }else{
          cat("The trial is ending, and we are declaring MTD for each cycles\n\n\n")
        }
      }else{
        if(length(act.index) != 0){
          cat(" Recommend dose for next cohort, and current paitents for next cycle based on longitudinal toxicity\n\n\n")
        } else {
          cat(" Recommend dose for next cohort based on longitudinal toxicity\n\n\n")
        }
      }
    ...
    # ##2. Recommend dose for next cohort, and current paitents for next cycle based on longitudinal toxicity and efficacy
    ...
    if(IED.flag == T){
      if(n.cohort < Max.cohort / 2){
        ...
        if(length(act.index) != 0){
          cat("Recommend dose for next cohort, and current patients for next cycle based on longitudinal toxicity and efficacy\n\n\n")
        } else{
          cat("Recommend dose for next cohort based on longitudinal toxicity and efficacy\n\n\n")
        }
      } else {
        if(length(act.index) != 0){
          cat("No more new cohort, recommend dose for current paitents for next cycle based on longitudinal toxicity and efficacy response\n\n\n")
        }else{
          cat("The trial is ending, and we are declaring efficacious doses, as well as recommending the lowest dose that is efficacious for all cycles\n\n\n")
        }
      }
    }else{
      if(n.cohort < Max.cohort / 2){
        ...
        cat("The trial is ending, and we are recommending the lowest dose that is efficacious for cycle 1\n\n\n")
      }
    }
  }
  ...
  pat_rec <- list(patID = patID_act, cycle = cycle_act + 1, dose = dose_act)
  set.seed(seed)
  ...
  if (n.cohort == 1) {
    ...
    if (sum(dlt) == 1) {
      ...
      cat("more than two 2 dlts out of 6 records in the first 2 cohort, stop the study\n\n\n")
      doseA <- NA
    }
    ...
    if(n.cohort < Max.cohort | ICD.flag == T){
      ...
      dose_flag <- ifelse(length(unique(patlist$dose)) == 1, 1, 0)
      ...
      if(dose_flag == 1){
        ...
        if(mnTTP.dose1 < p_tox1) {
          cat("early stop, the recommended dose is 1 \n\n\n") # need to be modified
          doseA <- 1
        } else {
          doseA <- doseA + 1    ## increase the dose for the next cohort
          pat_rec$patID <- patID_act[act.index]
          pat_rec$cycle <- cycle_act[act.index] + 1
          pat_rec$dose <- rep("Special cases", pat_rec$cycle)## ?????????? what should do for this cases
        }
      } else {
  ...
  }else if(n.cohort == Max.cohort & is.null(patID_act)){
    #############################
    ...
    if(IED.flag == T){
      ...
      pat_rec$cycle <- cycle_act[act.index]
      pat_rec$patID <- patID_act[act.index]
      pat_rec$dose <- rec_dose_act[act.index]
    }
  }
  cat("Recommend dose for cycle 1: ", doseA, "\n")
  cat("\nFor patients ", pat_rec$patID, "\non cycle ", pat_rec$cycle,
      "\nWe suggest dose levels: ", pat_rec$dose, "\n")
  res <- list(doseA = doseA, pat_rec = pat_rec)
  attr(res,'class') <- 'phase1model'
  return(res)
}
